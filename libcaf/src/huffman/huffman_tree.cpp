...
const uint64_t HuffmanTree::build_tree_and_get_root_index(const unsigned char * const str, size_t length) {
    assert(str != nullptr);
    assert(length > 0);

    std::array<uint64_t, 256> hist = histogram(str, length);
    std::priority_queue<const uint64_t, std::vector<const uint64_t>> min_heap;

    // create all leaf nodes and insert them into the minimum heap and the nodes data arrays
    uint64_t cur_node_index = 0;
    for (unsigned char i = 0; i < hist.size(); i++) {
        if (hist[i] > 0) {
            min_heap.push(cur_node_index);
    ...
    while (min_heap.size() > 1) {
        const uint64_t left_index = min_heap.top();
        min_heap.pop();
        const uint64_t right_index = min_heap.top();
        min_heap.pop();

        // create a new internal node
        const uint64_t parent_frequency = this->get_frequency(left_index) + this->get_frequency(right_index);
        this->add_node('\0', parent_frequency, left_index, right_index);
        const uint64_t parent_index = cur_node_index;
        cur_node_index++;
        min_heap.push(parent_index);
    }

    // the remaining node is the root
    const uint64_t root_index = min_heap.top();
    return root_index;
}
...
const unsigned char HuffmanTree::get_symbol(uint64_t index) const {
    return symbols_list[index];
}

const uint64_t HuffmanTree::get_frequency(uint64_t index) const {
    return frequencies_list[index];
}

const uint64_t HuffmanTree::get_left_index(uint64_t index) const {
    return left_indices_list[index];
}

const uint64_t HuffmanTree::get_right_index(uint64_t index) const {
